      ...
      <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-sx-12 pt-2 pl-4">
                ...
                <li class="breadcrumb-item active">Transport</ol>
            </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>
                ...
                <div class="text-middle">
                    ...
                    <thead class="" style="background-color:#ebeef3;color:#7F92A3"> 
                        <tr>
                            ...
                            <th width="20%">ชื่อการขนส่ง</th>
                            <th width="55%">เรทราคา</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-container">
                ...
                <form id="theForm" action="{{ url('admin/transport') }}" method="POST" typeForm="" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="name">ชื่อการขนส่ง</label>
                            <input type="text" class="form-control btn_rounded" id="name" name="name" aria-describedby="name"
                                placeholder="ชื่อการขนส่ง">
                        </div>
                        <div class="form-group">
                            <label for="rate">เรทราคา</label>
                            <input type="number" class="form-control btn_rounded" id="rate" name="rate" aria-describedby="rate"
                                placeholder="เรทราคา">
                        </div>
                        <div class="form-group">
    ...
    if (theForm.length > 0) {
        theForm.validate({
            ...
            messages: {
                ...
                rate: {
                    required: "Please enter transport rate",
                },
            },
            errorElement: 'span',
        ...
        $.ajax({
            ...
            success: (response) => {
                if ($.isEmptyObject(response.error)) {
                    ...
                    $('.modal-title').html("แก้ไขการขนส่ง");
                    ...
                    $('#name').val(response.transport);
                    $('#rate').val(response.transport_rate);
                    $('#description').html("");
                    tinymce.activeEditor.setContent(response.transport_description ?? '');
                    tinyMCE.triggerSave();
        ...
        table = $("#example").DataTable({
            ...
            buttons: [{
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        $('.modal-title').html("เพิ่มการขนส่ง");
            ...
            columns: [{
                ...
                {
                    data: ({transport}) => transport ? `<strong class="text-color_main">${transport}</strong>` : "-",
                    name: 'transport'
                },
                {
                    data: ({transport_rate}) => transport_rate ? `<strong class="text-color_main">${transport_rate}</strong>` : "-",
                    name: 'transport_rate'
                },
                {
                    data: 'action',
